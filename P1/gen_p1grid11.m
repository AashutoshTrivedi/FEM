function [ne,np,p,conn,gbc] = gen_p1grid11(nref)
% define an initial coarse mesh
ne = 4;
x(1,1)= 0; y(1,1)= 0 ; efl(1,1)=0;
x(1,2)= 1000.0; y(1,2)= -1000.0; efl(1,2)=1;
x(1,3)= 1000.0; y(1,3)= 1000.0; efl(1,3)=1; % 0 for inner node
% 1 for Dirichlet node
x(2,1)= 0; y(2,1)= 0; efl(2,1)=0;
x(2,2)= 1000.0; y(2,2)= 1000.0; efl(2,2)=1;
x(2,3)= -1000.0; y(2,3)= 1000.0; efl(2,3)=1; % second element
x(3,1)= 0; y(3,1)= 0; efl(3,1)=0;
x(3,2)= -1000.0; y(3,2)= 1000.0; efl(3,2)=1;
x(3,3)= -1000.0; y(3,3)= -1000.0; efl(3,3)=1; % third element
x(4,1)= 0; y(4,1)= 0; efl(4,1)=0;
x(4,2)= -1000.0; y(4,2)= -1000.0; efl(4,2)=1;
x(4,3)= 1000.0; y(4,3)= -1000.0; efl(4,3)=1; % fourth element
...
xn(nm,3)=x(j,3);
%3rd sub-element
yn(nm,1)=y(j,6); efln(nm,1)=efl(j,6);
yn(nm,2)=y(j,5); efln(nm,2)=efl(j,5);
yn(nm,3)=y(j,3); efln(nm,3)=efl(j,3);
nm = nm+1;
% 4th sub-element
xn(nm,1)=x(j,4); yn(nm,1)=y(j,4); efln(nm,1)=efl(j,4);
...
np=3;
% we already has 3 nodes from 1st element!
...
for k=1:np
  ...
  Id = 1;
  % indicate this node has already been used
